...
//i is the index of the key in A
...
//===========the above is for the heap========================

struct Vertex
{
	int id;//the id of the vertex
	int weight; //the weight of an edge (u,v), where v is the current node and v is in the adjlist of vertex u
	Vertex * next;
...
class Graph
{
	...
	public:
		...
		void Dijkstra(int s);//compute a shortest path tree from source s in a general graph

		void printSP(int source, int v);//print shortest path from the source to v
		int getSPdis(int v);//get shortest path distance from source to v

...
void AvlTree::balance(AvlNode*& v) {
	...
	if(balanceFactor > 1) {
		...
		balanceFactor = getHeight(v->left->left) - getHeight(v->left->right);

		if(balanceFactor < 0) { leftRotate(v->left); }
		rightRotate(v);
	...
	//Tree is right heavy.
		...
		balanceFactor = getHeight(v->right->left) - getHeight(v->right->right);

		if(balanceFactor > 0) { rightRotate(v->right); }
		leftRotate(v);

	}

	v->height = 1 + ( (getHeight(v->left) > getHeight(v->right)) ? 
		getHeight(v->left) : getHeight(v->right) );
	return;
}

void AvlTree::rightRotate (AvlNode*& v) {

	AvlNode* partner = v->left;

	//Partner becomes the new root.
	swap(*v, *partner);

	//Former root takes on lower subtree.
	partner->left = v->right;

	//New Root takes ownership of old root.
	v->right = partner;

	partner->height = 1 + ( (getHeight(partner->left) > getHeight(partner->right)) ? 
		getHeight(partner->left) : getHeight(partner->right) );
...
void AvlTree::leftRotate (AvlNode*& v) {

	AvlNode* partner = v->right;

	//Partner becomes the new root;
	swap(*v, *partner);

	//Former root takes on the lower subtree.
	partner->right = v->left;

	//New Root takes ownership of the old root.
	v->left = partner;
